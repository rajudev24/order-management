...
import ProductAddEditPopup from "./ProductAddEditPopup.jsx";
...
const ProductsPage = () => {
  ...
  const handleUpdateStatus = async (itemId, row_status) => {
    try {
      ...
      successStack("Product status updated Successfully", "success");
    } catch (error) {
  ...
  const handleDelete = async (itemId) => {
    try {
      ...
      successStack("Product Deleted Successfully", "success");
      await mutateProduct();
    } catch (error) {
      errorStack(error?.response?.data?.message, "error");
    }
